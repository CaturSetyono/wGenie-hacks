...
interface Props {
  ...
  vaultsData: MantleVaultData[] | undefined;
...
export function AllocationTable({ chainId, positions, vaultsData, prices, isLoading }: Props) {
  if (isLoading) {
    return (
      <div className="bg-wgenie-dark rounded-lg border border-white/5 p-4">
  ...
  // Merge on-chain positions with Mantle vault API data
  ...
  return (
    <div className="bg-wgenie-dark rounded-lg border border-white/5 overflow-hidden">
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-wgenie-muted text-[11px] uppercase tracking-wider">
              ...
              >
                ...
                <td className="py-3 pl-4">
                  ...
                  >
                    ...
                    <span className="text-wgenie-muted text-xs">
                      {row.underlying}
                    </span>
                  </Link>
                </td>
                ...
                <td className="py-3 text-right">
                  ...
                  {unallocAmount > 0 && unallocUsd !== null && (
                    <div className="font-mono text-[10px] text-wgenie-muted">
                ...
                <td className="py-3 text-right">
                  <div
                    className={`font-mono text-xs ${
                      isActive ? 'text-white' : 'text-wgenie-muted'
                    }`}
                  >
                    {isActive
      ...
      {!hasAnyPosition && (
        <div className="px-4 pb-4 pt-2 text-center">
          <p className="text-xs text-wgenie-muted">
            No allocations yet. Use the AI copilot below to allocate funds to Mantle vaults.
          </p>
        </div>
      )}
    </div>
