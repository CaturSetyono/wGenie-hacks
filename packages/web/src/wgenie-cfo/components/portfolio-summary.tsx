...
function StatCard({
  ...
  return (
    <div className="relative bg-wgenie-dark rounded-lg p-4 border border-white/5 overflow-hidden group">
      ...
      <div className="flex items-center gap-2 mb-2">
        <div className={`p-1.5 rounded-md ${accent ? 'bg-primary/10' : 'bg-white/5'}`}>
          <Icon className={`w-3.5 h-3.5 ${accent ? 'text-primary' : 'text-wgenie-muted'}`} />
        </div>
      ...
      <div className={`text-xl font-semibold tracking-tight ${accent ? 'text-primary' : 'text-white'}`}>
        {value}
      </div>

      {subValue && (
        <div className="text-xs text-wgenie-muted mt-0.5">{subValue}</div>
      )}
    </div>
  );
}
    ...
    return (
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="bg-wgenie-dark rounded-lg p-4 border border-white/5 animate-pulse"
          >
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      ...
      <div className="relative bg-wgenie-dark rounded-lg p-4 border border-white/5 overflow-hidden group">
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none bg-primary/[0.02]" />
        <div className="flex items-center gap-2 mb-2">
          <div className={`p-1.5 rounded-md ${isConnected && hasAlphaRole ? 'bg-primary/10' : 'bg-white/5'}`}>
            <Shield className={`w-3.5 h-3.5 ${isConnected && hasAlphaRole ? 'text-primary' : 'text-wgenie-muted'}`} />
          </div>
          <span className="text-[11px] font-medium tracking-wider uppercase text-wgenie-muted">
            Alpha Role
          </span>
        ...
        <div className={`text-xl font-semibold tracking-tight ${
          !isConnected ? 'text-wgenie-muted'
            : isRoleLoading ? 'text-wgenie-muted'
            : hasAlphaRole ? 'text-primary'
            : 'text-white'
        }`}>
          {!isConnected
            ? 'Connect wallet'
        ...
        <a
          ...
          className="text-xs text-wgenie-muted hover:text-primary transition-colors mt-0.5 inline-block"
        >
          Manage roles &rarr;
        </a>
      </div>
