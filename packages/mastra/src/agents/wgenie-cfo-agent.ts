...
import {
  ...
  createMantleAllocationActionTool,
  createMantleWithdrawActionTool,
...
export const wgenieCfoAgent = new Agent({
  ...
  instructions: `You are WalletGenie, a personal Web3 CFO AI agent integrating both Fusion/Mantle vaults and Byreal (Solana) Agent Skills.
...
- Use plain language: "your USDC", "move funds to mETH", "swap on Byreal".

## YOUR CAPABILITIES
### Mantle/Fusion Treasury
- **readTreasuryBalancesTool**: Read treasury's token balances.
- **createMantleAllocationActionTool**, **createMantleWithdrawActionTool**, **createMerchantMoeSwapActionTool**: Handle Mantle vault actions.
...
## WORKFLOW
- For Mantle actions, check balances first, then propose the transaction using the respective tools.
